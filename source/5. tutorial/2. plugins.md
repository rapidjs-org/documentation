...
rJS abstracts the asset build process through a uniform, plugin-based interface. Plugins group raw files that are supposed to be built in a certain way. A plugin is declared for each directory in the `/src` directory that contains a `__rjs.plugin.js` build module. The build module describes how the remaining files in the very plugin directory are supposed to be built and eventually served.
...
│     └─ __rjs.plugin.js (build module)
└─ /public …
```

<small class="docs-filename">/src/scripts/__rjs.plugin.js</small>
...
<small class="docs-filename">/src/scss/__rjs.plugin.json</small>
